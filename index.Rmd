...
However, it is interesting to observe firsthand that there is one song that is an extreme different from the rest, which is none other than the outlier track Light Rain, which is the song that I listen to sleep in the past and that comprises of basically rain sounds.
...
The sheds light that a language agnostic algorithm can in fact bring music of languages together and cogently display how music can unite us rather than divide us. That we can listen to different languages with similar styles, and that we don't always need to be confined to our own cultures, that we can embrace something new. If we close our eyes to the language of the lyrics and open our eyes to what music can do to our hearts, we can possibly create a more inclusive world, one that our hearts yearn to see in a world that is currently going through turmoil, unrest and division.
...
# geom_text(aes(x=0.14, label=ifelse((instrumentalness>0.5),as.character(name),'')))
  ...
  labs(x = "Valence", y = "Instrumentalness", title="Analysis of Valence and Instrumentalness of Megan and Si Han's Playlists") 
```
