...
p <- ggplot(combined_df, aes(valence, instrumentalness)) +
  geom_point(aes(color=mode)) +
  facet_wrap(~listener) +
  # geom_text_repel(aes(label=ifelse((instrumentalness>0.85),as.character(name),''))) +
  geom_text(aes(x=0.14, label=ifelse((instrumentalness>0.97),as.character(name),'')))
